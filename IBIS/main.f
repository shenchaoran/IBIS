...
! region argv scr
     ...
     & path must as a prefix of "--tag="!'
          ...
          if(fileTag == '--input=' .or. fileTag == '-i=') then
              infile = tempStr(filePathStart+1:)
          elseif(fileTag == '--output=' .or. fileTag == '-o=') then
              outfile = tempStr(filePathStart+1:)
          elseif(fileTag == '--site=' .or. fileTag == '-s=') then
              siteinfofile = tempStr(filePathStart+1:)
          end if
          print *,tempStr
      end do
      
      if(len_trim(siteinfofile) == 0 .or.
     > len_trim(infile) == 0 .or. 
     > len_trim(outfile) == 0 ) then
          print 377, 'invalid input file!'
          stop
      end if
      ! endregion

      !open(11,file="C:\IBIS\Data\Site_Info_File_run8.txt",action="read")
      !open(12,file="C:\IBIS\Data\Site_Cli_File_run8.txt",action="read")
      !open(13,file="C:\IBIS\Data\Site_Output_File_run8.txt",action="read")
