...
c
c ---------------------------------------------------------------------
      subroutine daily (imonth, iday, seed, jdaily)
c ---------------------------------------------------------------------
c
c overview
c
...
c
          pwet = max (1., xinwet(imonth)) / ndaypm(imonth)
c
...
c
            xinprec(imonth) = max(0., xinprec(imonth))
            xinwet(imonth) = max(1., xinwet(imonth))
c
 9000       continue
c
...
c
                if (iwetday(j-1) .eq. 0) then
                  if (rndnum.le.pwd) then
                    iwetday(j) = 1
                    iwetdaysum = iwetdaysum + 1
                  endif
                else
                  if (rndnum.gt.pww) iwetday(j) = 0
                endif
              endif
c
c ---------------------------------------------------------------------- 
...
c
              if (iwetday(j) .eq. 1) then
c
c calculate average rainfall per wet day
c
                rainpwd = xinprec(imonth) * ndaypm(imonth) /
     >                max (0.1, xinwet(imonth))
c
...
c
                precipday(j) = max (precipday(j),0.25) ! min =   0.25 mm/day
                precipday(j) = min (precipday(j),150.00) ! max = 150.00 mm/day
c
...
c
              precipdaysum = precipdaysum + precipday(j)
c
 220        continue
c
...
c
c
              if ((precipdaysum .eq. 0) .AND.
     >                (xinprec(imonth) .gt. 0.)) then
                    rndnum = 1.0 + (float(ndaypm(imonth)) - 1.0)
     >                   * ran2(seed)
                    iwetday(nint(rndnum)) = 1
                    precipday(nint(rndnum)) = xinprec(imonth)
     >                   * float(ndaypm(imonth))
                    precipdaysum = precipday(nint(rndnum))
                    iwetdaysum = 1
                 end if
c
                 precipfac = (xinprec(imonth)*float(ndaypm(imonth)))
     >                / max(0.01,precipdaysum)
c
                 do 230 j=1,ndaypm(imonth)
                    precipday(j) = precipday(j) * precipfac
c
                    if (precipday(j).gt.360) then
                       if (xinwet(imonth) .lt. ndaypm(imonth)) then
                          xinwet(imonth) = xinwet(imonth) + 1
                          pwet = xinwet(imonth) / ndaypm(imonth)
                          pwd = 0.75 * pwet
                          pww = 0.25 + pwd
                          print *, 'WARNING: goto 9000a', int(xinwet(imonth)), iwetdaysum, int(precipday(j))
                          goto 9000
                       else
                          print *, 'WARNING: goto 9000b', int(xinwet(imonth)), iwetdaysum, int(precipday(j))
                          goto 9000
                       end if
                    end if
c    
...
c
                 do 240 j=1,ndaypm(imonth)
                    precipdaysum = precipdaysum + precipday(j)
                    iwetdaysum = iwetdaysum + iwetday(j)
  240            continue
c
                 dif = precipdaysum - xinprec(imonth)
     >                   * float(ndaypm(imonth))
c    
                 if ((dif.lt.-0.1).or.(dif.gt.0.1)) print *,
     >                'ERROR in DAILY:', precipdaysum,
     >                xinprec(imonth)* float(ndaypm(imonth)),
     >                iwetdaysum, xinwet(imonth)
c    
...
c
          tdm = xint(it1w) +
     >          dt * (xint(it2w) - xint(it1w)) + 273.16
c
c mean daily temperature range (K)
c
          trngm = xintrng(it1w) +
     >            dt * (xintrng(it2w) - xintrng(it1w))
c
...
c
          if (iwet.eq.0) tmaxe = tmaxd
          if (iwet.eq.1) tmaxe = tmaxw
c
          tmine = tminm
c
...
c
          if (iday.eq.1) then
             xincld(it1w) = max (0., xincld(it1w))
             xincld(it1w) = min (100., xincld(it1w))
             xincld(it2w) = max (0., xincld(it2w))
             xincld(it2w) = min (100., xincld(it2w))
          end if

c monthly mean cloud cover (%)
c
          cloudm = xincld(it1w) + dt * (xincld(it2w) - xincld(it1w))
c 
...
c
          if (iwet.eq.0) cloude = cloudd
          if (iwet.eq.1) cloude = cloudw
c
...
c 
          if (iwet.eq.0) clouds = 0.24 * cloude
          if (iwet.eq.1) clouds = 0.48 * cloude
c
c ---------------------------------------------------------------------- 
...
c
          tmax  = tmaxe + tmaxs * x(1)
          tmin  = tmine + tmins * x(2)
c
...
c
          td = 0.44 * tmax + 0.56 * tmin
c
c determine today's cloud cover
c
          cloud = cloude + clouds * x(3)
c
...
c
          psurf = 101325.0 * (td / (td + 0.0065 * xintopo)) ** rwork
c
c ---------------------------------------------------------------------- 
...
c
          if (iday.eq.1) then
             xinq(it1w) = max (0., xinq(it1w))
             xinq(it1w) = min (100., xinq(it1w))
             xinq(it2w) = max (0., xinq(it2w))
             xinq(it2w) = min (100., xinq(it2w))
          end if
c
c mean relative humidity (%)
c
          qdm = xinq(it1w) + dt * (xinq(it2w) - xinq(it1w))
c 
c convert from percent to fraction
c
          qdm = qdm / 100.0
...
c
          if (iwet.eq.0) qde = qdd
          if (iwet.eq.1) qde = qdw 
c
...
c
          if (rn.gt.x1) then
            qd = qdup  - sqrt (b2 * b2 - 2.0 * b2 * (b1 - 0.5 * b3))
          else
            qd = qdlow + sqrt (2.0 * b1 * b3)
          endif
c
...
c
          precip = (xinprec(imonth) * ndaypm(imonth)) * xinprecd
...
c
          td = xint(imonth) + 273.16 + xintd
          trngm = min (44.0, (xintrng(imonth) * xintrngd))
c
          tmax = td + 0.56 * trngm
          tmin = td - 0.44 * trngm
c
...
c
          cloud = (xincld(imonth) + xincldd) * 0.01
c
          cloud = min (cloud, 1.)
          cloud = max (0.0, cloud)
c
...
c anomaly
c
          humidfrac = xinq(imonth) / 100.
          sphumid = humidfrac * qsat(esat(td),psurf)
c
          qd = sphumid * xinqd
c
c daily average wind speed
...
c
c ---------------------------------------------------------------------
      subroutine diurnal (iyear,lat,time, jday, plens, startp, endp, seed)
c ---------------------------------------------------------------------
c
c common blocks
c
...
c        jj = latindex(i)
c
c             xlat = latscale(jj) * pi / 180.0

        xlat = lat * pi / 180.0
c
c calculate the cosine of the solar zenith angle
c
        coszen = max (0.0, (sin(xlat) * sin(xdecl) +
...
c
          solad(ib) = sw * coszen * frac * trans * (1. - fdiffuse)  
c
          solai(ib) = sw * coszen * frac * trans * fdiffuse
...
c
        qa = min (qsa, qmax * gamma + qmin * (1.0 - gamma))
c
c calculate the hourly relative humidity 
c
        rh = 100.0 * qa / qsat(esat(ta), psurf)
c
c ---------------------------------------------------------------------- 
...
c
        ...
        ea  = 0.70 + 5.95e-5 * emb * exp (1500.0 / ta)
c
...
c
        fira = (1. -  cloud) * ea * stef * (ta - dtair  )**4 +
     >                   cloud  * ec * stef * (ta - dtcloud)**4
c
c ---------------------------------------------------------------------- 
...
c
           ...
           checkP = 0
c
           do  while (((precip/plen) .gt. 15).and.(plen.lt.plenmax))
              plen = plen + 1
              checkP = 1
           end do
c
