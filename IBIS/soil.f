
c  ####    ####      #    #
c #       #    #     #    #
c  ####   #    #     #    #
c      #  #    #     #    #
c #    #  #    #     #    #
c  ####    ####      #    ######
c
c ---------------------------------------------------------------------
      subroutine setsoi
c ---------------------------------------------------------------------
c
c sets diagnostic soil quantities
c
      include 'implicit.h'
c
      include 'compar.h'
      include 'comatm.h'
      include 'comsno.h'
      include 'comsoi.h'
      include 'comveg.h'
c
c Local variables
c
      integer k,           ! loop indices
     >        msand,          ! % of sand in grid point
     >        mclay           ! % of clay in grid point
c
      real fsand,             ! fraction of sand in grid point
     >     fsilt,             ! fraction of silt in grid point
     >     fclay,             ! fraction of clay in grid point
* MEM: added forganic for organic soils.
cc     >     forganic,          ! fraction of organic soil in grid point
     >     powliq,            ! liquid water content in fraction of soil depth
     >     powice,            ! ice water content in fraction of soil depth
     >     zcondry,           ! dry-soil conductivity
     >     zvap,              ! latent heat of vaporisation at soil temp
     >     zsub,              ! latent heat of sublimation at soil temp
     >     zwpud,             ! fraction of soil surface covered by puddle
*     >     zwpmax,            ! assumed maximum value of zwpud
     >     zwsoi,             ! volumetric water content of top soil layer 
     >     rwork1,
     >     rwork2
c
      include 'comsat.h'    
c
c set soil layer quantities
c
      do 100 k = 1, nsoilay
c
cc        do 110 i = 1, npoi
c
c Convert input sand and clay percents to fractions
c
          msand = nint(sand(k))
          mclay = nint(clay(k))
c
          fsand = 0.01 * msand
          fclay = 0.01 * mclay
          fsilt = 0.01 * (100 - msand - mclay)
c
c update thermal conductivity (w m-1 k-1)
c
c based on c = c1**v1 * c2**v2 * c3**v3 * c4**v4 where c1,c2..
c are conductivities of soil grains, air, liquid and ice
c respectively, and v1,v2... are their volume fractions 
c (so v1 = 1-p where p is the porosity, and v1+v2+v3+v4 = 1).
c then condry = c1**(1-p) * c2**p  is the dry-soil
c conductivity, and c = condry * (c3/c2)**v3 * (c4/c2)**v4, 
c where c2 = conductivity of air = .025 w m-1 k-1.
c however this formula agrees better with williams+smith
c table 4 for wet (unfrozen) sand and clay if c2 is decreased
c to ~.005. (for peat in next section, ok if c2 = .025).
c also see lachenbruch etal,1982,jgr,87,9301 and refs therein.
c
          powliq = poros(k) * wsoi(k) * (1. - wisoi(k))
          powice = poros(k) * wisoi(k)
c
          zcondry = fsand * 0.300 +
     >              fsilt * 0.265 +
     >              fclay * 0.250 ! +
* M. El Maayar added this to account for contribution of organic soils
c     >              forganic * 0.026
c
          consoi(k) = zcondry * ((0.56*100.)**powliq) * ((2.24*100.)**powice)
c
cc 110    continue
c
 100  continue
c
c set qglif - the fraction of soil sfc evaporation from soil liquid,
c soil ice, puddle liquid, and puddle ice (relative to total sfc evap)
c
c zwpud:   fraction of surface area covered by puddle (range: 0 - zwpmax)
c zwpmax:  maximum value of zwpud (currently assumed to be 0.5)
c 1-zwpud: fraction of surface area covered by soil (range: (1-zwpmax) - 1.0)
c zwsoi:   volumetric water content of top soil layer (range: 0 - 1.0)
c
c qglif(1): fraction of soil evap (fvapg) from soil liquid
c qglif(2): fraction of soil evap (fvapg) from soil ice
c qglif(3): fraction of soil evap (fvapg) from puddle liquid
c qglif(4): fraction of soil evap (fvapg) from puddle ice
c
cc      do 200 i = 1, npoi
c
*        zwpmax = 0.5
        zwpud = max (0.0, min (zwpmax, zwpmax*(wpud+wipud)/wpudmax) )
        zwsoi = (1. - wisoi(1)) * wsoi(1) + wisoi(1)
c
        if (zwsoi.ge.epsilon) then
c
          rwork1 = 1./zwsoi
c
          if (zwpud.ge.epsilon) then
            rwork2 = 1./(wpud + wipud)
            qglif(1) = (1. - zwpud) * (1. - wisoi(1)) * wsoi(1) * rwork1
            qglif(2) = (1. - zwpud) * wisoi(1) * rwork1
            qglif(3) = zwpud * wpud * rwork2
            qglif(4) = zwpud * wipud * rwork2
          else
            qglif(1) = (1. - wisoi(1)) * wsoi(1) * rwork1
            qglif(2) = wisoi(1) * rwork1
            qglif(3) = 0.0
            qglif(4) = 0.0
          endif
c
        else
c
c for a 100% dry soil surface, assign all soil evap to the puddles.
c Note that for small puddle sizes, this could lead to negative
c puddle depths. However, for a 100% dry soil with small puddles,
c evaporation is likely to be very small or less than zero
c (condensation), so negative puddle depths are not likely to occur.
c
          if (zwpud.ge.epsilon) then
            rwork2 = 1./(wpud + wipud)
            qglif(1) = 0.0
            qglif(2) = 0.0
            qglif(3) = zwpud * wpud * rwork2
            qglif(4) = zwpud * wipud * rwork2
          else
            if (tsoi(1).ge.tmelt) then
c
c above freezing
c
              qglif(1) = 0.
              qglif(2) = 0.
              qglif(3) = 1.
              qglif(4) = 0.
c
            else
c
c below freezing
c
              qglif(1) = 0.
              qglif(2) = 0.
              qglif(3) = 0.
              qglif(4) = 1.
            endif
          endif
c
        endif
c
c set latent heat values
c
        zvap = hvapf (tsoi(1), ta)
        zsub = hsubf (tsoi(1), ta)
c
        hvasug = (qglif(1) + qglif(3)) * zvap +
     >              (qglif(2) + qglif(4)) * zsub 
c
        hvasui = hsubf(tsno(1),ta)
c
cc 200  continue   
c
      return
      end
c
c
c ---------------------------------------------------------------------
      subroutine soilctl
c ---------------------------------------------------------------------
c
c steps soil/seaice model through one timestep
c
      include 'implicit.h'
c
      include 'compar.h'
      include 'comhyd.h'
      include 'comatm.h'
      include 'comsno.h'
      include 'comsoi.h'
      include 'comveg.h'
      include 'com1d.h'
c
      integer k          ! loop indices
c
      real zfrez,           ! factor decreasing runoff fraction for tsoi < tmelt
     >     zrunf,           ! fraction of rain that doesn't stay in puddle (runoff fraction)
     >     rwork,           ! 
     >     wipre,           ! storing variable
     >     zdpud,           ! used to compute transfer from puddle to infiltration
     >     cx,              ! average specific heat for soil, water and ice
     >     chav,            ! average specific heat for water and ice
     >     zwsoi      
c
      real
     >  owsoi(nsoilay),    ! old value of wsoi
     >  otsoi(nsoilay),    ! old value of tsoi
     >  c0pud(nsoilay),    ! layer heat capacity due to puddles (=0 except for top)
     >  c1pud(nsoilay),    ! updated av. specifilayer heat capacity due to  puddle
     >  wflo(nsoilay+1),   ! = drainage at the bottom, returned by soilh2o
     >  fwtop,            ! evaporation rate from soil (for soilh2o)
     >  fhtop,            ! heat flux through soil surface (for soilheat)
     >  fwpud,            ! portion of puddle that infiltrates in soil (rate)
     >  fsqueez,          ! excess amount of water (soilh2o) 
     >  dh,               ! correction if water at tsoi < tmelt or ice at temp > tmelt
     >  dw,               ! '
     >  zporos            
c
      include 'comsat.h'
c
      call const (c0pud, nsoilay, 0.0)
      call const (c1pud, nsoilay, 0.0)
c
c for soil, set soil infiltration rate fwtop (for 
c soilh2o) and upper heat flux fhtop (for soilheat)
c
c also step puddle model wpud, wipud
c
c procedure is:
c
c   (0) immediately transfer any excess puddle liq to runoff
c
c   (1) apportion raing btwn puddle liquid(wpud) or runoff(grunof)
c
c   (2) apportion evap/condens (fvapg) btwn infil rate(fwtop), soil
c       ice (wisoi(1)), puddle liq (wpud), or puddle ice (wipud)
c
c   (3) transfer some puddle liquid to fwtop
c
c   (4) compute upper heat flx fhtop: includes fwtop*ch2o*tsoi(1)
c       to be consistent with whflo in soilheat, and accounts for
c       changing rain temp from traing to tsoi(1) and runoff temp
c       from tsoi to max(tsoi(1),tmelt)
c
cc      do 100 i = 1, npoi
c
c (0) immediately transfer any excess puddle liq to runoff
c
c the following runoff formulation could give rise to very
c small amounts of negative runoff
c
        grunof = min (wpud, max (0., wpud + wipud -
     >                                     wpudmax)) / dtime
c
        wpud = wpud - grunof * dtime
c
c (1) apportion sfc-level rain between puddle liquid and runoff
c
c linear dependence of runoff fraction on wpud+wipud assumes
c uniform statistical distribution of sub-grid puddle 
c capacities between 0 and wpudmax. runoff fraction is 
c reduced linearly for tsoi < tmelt (by zfrez) in which case
c any rain will increase wpud which will be frozen to wipud
c below
c
        zfrez = max (0., min (1., (tsoi(1) - tmelt + .5) * 2.))
c
c always have some minimal amount of runoff (0.10) even if 
c puddles are dry or soil is cold
c
        zrunf = zfrez * max (0., min (1., (wpud + wipud) / 
     >                                     wpudmax))
c
        wpud = wpud + (1. - zrunf) * raing * dtime
c
        grunof = grunof + zrunf * raing
c
c (2) apportion evaporation or condensation between 4 h2o stores:
c
        rwork = fvapg * dtime
c
        if (fvapg.ge.0.) then
c
c evaporation: split according to qglif
c
          fwtop   =            - qglif(1)*fvapg
          wpud    = wpud    - qglif(3)*rwork
          wipud   = wipud   - qglif(4)*rwork
c
          wipre = wisoi(1)
          wisoi(1) = max (0., wipre - qglif(2)*rwork /
     >                          (rhow*poros(1)*hsoi(1)))
c
          if (1.-wisoi(1).gt.epsilon)
     >      wsoi(1) = wsoi(1)*(1.-wipre)/(1.-wisoi(1))
c
        else
c
c condensation: give all to puddles (to avoid wsoi, wisoi > 1)
c
          fwtop = 0.
          wpud = wpud  - (qglif(1)+qglif(3))*rwork
          wipud= wipud - (qglif(2)+qglif(4))*rwork
c
        endif
c
c (3) transfer some puddle liquid to infiltration; can lead
c     to small amounts of negative wpud (in soilh2o) due to
c     round-off error
c
        zdpud = rhow * dtime * max (0., 1.-wisoi(1))**2 *
     >          hydraul(1)
c
        fwpud = max (0., min (wpud, zdpud)) / dtime
        c0pud(1) = ch2o*wpud + cice*wipud
c
c (4) compute upper soil heat flux
c
        fhtop = heatg
     >           + raing*ch2o*(traing-tsoi(1))
     >           - grunof*ch2o*max(tmelt-tsoi(1), 0.)
c
c update diagnostic variables
c
        gadjust = 0.0
c
cc 100  continue
c
c reduce soil moisture due to transpiration (upsoi[u,l], from
c turvap).need to do that before other time stepping below since 
c specific heat of this transport is neglected
c
c first set porosflo, reduced porosity due to ice content, used
c as the effective porosity for uptake here and liquid hydraulics
c later in soilh2o. to avoid divide-by-zeros, use small epsilon
c limit; this will always cancel with epsilon or 0 in numerators
c
c also increment soil temperature to balance transpired water
c differential between temps of soil and leaf. physically
c should apply this to the tree, but would be awkward in turvap.
c 
c also, save old soil moisture owsoi and temperatures otsoi so
c implicit soilh2o and soilheat can aposteriori deduce fluxes.
c
      do 120 k = 1, nsoilay
c
cc        do 130 i = 1, npoi
c
          porosflo(k) = poros(k) * max (epsilon, (1.-wisoi(k)))
c
c next line just for ice whose poros(k) is 0.0
c
          porosflo(k) = max (porosflo(k), epsilon)
c
          wsoi(k) = wsoi(k) - dtime * (upsoiu(k) + upsoil(k)) /
     >                            (rhow * porosflo(k) * hsoi(k))
c
          cx = c0pud(k) + 
     >         (   (1.-poros(k))*csoi(k)*rhosoi(k)
     >           + poros(k)*(1.-wisoi(k))*wsoi(k)*ch2o*rhow
     >           + poros(k)*wisoi(k)*cice*rhow
     >         ) * hsoi(k)
c
          tsoi(k) = tsoi(k) - dtime * ch2o * 
     >                (  upsoiu(k)*(tu-tsoi(k))
     >                 + upsoil(k)*(tl-tsoi(k)) ) / cx
c
          owsoi(k)  = wsoi(k)
          otsoi(k)  = tsoi(k)
c
cc 130    continue
c
 120  continue
c
c step soil moisture calculations
c
      call soilh2o (owsoi, fwtop, fwpud, fsqueez, wflo)
c
c update drainage and puddle
c
cc      do 200 i = 1, npoi
c
        gdrain  = wflo(nsoilay+1)
        c1pud(1) = ch2o*wpud + cice*wipud
c
cc 200  continue
c
c step temperatures due to conductive heat transport
c
      call soilheat (otsoi, owsoi, c0pud, fhtop, wflo, c1pud)
c
c set wsoi, wisoi to exactly 0 or 1 if differ by negligible 
c amount (needed to avoid epsilon errors in loop 400 below)
c
      call wadjust
c
c heat-conserving adjustment for liquid/ice below/above melt
c point. uses exactly the same logic as for intercepted veg h2o
c in steph2o2. we employ the fiction here that soil liquid and
c soil ice both have density rhow, to avoid "pot-hole"
c difficulties of expansion on freezing. this is done by 
c dividing all eqns through by rhow(*hsoi).
c
c the factor (1-wsoi(old))/(1-wisoi(new)) in the wsoi increments
c results simply from conservation of h2o mass; recall wsoi is
c liquid content relative to ice-reduced pore space.
c
      do 400 k = 1, nsoilay
cc        do 410 i = 1, npoi
c
c next line is just to avoid divide-by-zero for ice with
c poros = 0
c
          zporos = max (poros(k), epsilon)
          rwork = c1pud(k)/rhow/hsoi(k)
     >           + (1.-zporos)*csoi(k)*rhosoi(k)/rhow
c
          chav = rwork
     >           + zporos*(1.-wisoi(k))*wsoi(k)*ch2o
     >           + zporos*wisoi(k)*cice
c
c if liquid exists below melt point, freeze some to ice
c
c (note that if tsoi>tmelt or wsoi=0, nothing changes.)
c (also note if resulting wisoi=1, either dw=0 and prev
c wisoi=1, or prev wsoi=1, so use of epsilon is ok.)
c
          zwsoi = min (1., wsoi(k))
c
          dh = chav * (tmelt-tsoi(k))
          dw = min ( zporos*(1.-wisoi(k))*zwsoi,
     >                  max (0.,dh/hfus) )
c
          wisoi(k) = wisoi(k) +  dw/zporos
          wsoi(k)  = wsoi(k)  - (dw/zporos)*(1.-zwsoi)
     >                              / max (epsilon,1.-wisoi(k))
c
          chav = rwork
     >           + zporos*(1.-wisoi(k))*wsoi(k)*ch2o
     >           + zporos*wisoi(k)*cice
c
          tsoi(k) = tmelt - (dh-hfus*dw) / chav
c
c if ice exists above melt point, melt some to liquid
c
c note that if tsoi<tmelt or wisoi=0, nothing changes
c
c also note if resulting wisoi=1, dw=0 and prev wisoi=1,
c so use of epsilon is ok
c
          dh = chav * (tsoi(k) - tmelt)
          dw = min ( zporos*wisoi(k), max (0., dh/hfus) )
c
          wisoi(k) = wisoi(k) -  dw/zporos
          wsoi(k)  = wsoi(k)  + (dw/zporos)
     >                 * (1.-wsoi(k)) / max(epsilon,1.-wisoi(k))
c
          chav = rwork
     >           + zporos*(1.-wisoi(k))*wsoi(k)*ch2o 
     >           + zporos*wisoi(k)*cice
c
          tsoi(k) = tmelt + (dh-hfus*dw) / chav
c
c reset porosflo (although not used after this)
c
          porosflo(k) = zporos * max (epsilon, 1.-wisoi(k))
c
cc  410   continue
  400 continue
c 
c set wsoi, wisoi to exactly 0 or 1 if differ by negligible 
c amount (roundoff error in loop 400 above can produce very
c small negative amounts)
c
      call wadjust
c
c repeat ice/liquid adjustment for upper-layer puddles (don't 
c divide through by rhow*hsoi). upper-layer moistures wsoi,wisoi
c are already consistent with tsoi(1) > or < tmelt, and will 
c remain consistent here since tsoi(1) will not cross tmelt
c
      k = 1
c
cc      do 500 i = 1, npoi
c
c if any puddle liquid below tmelt, freeze some to puddle ice
c
        rwork = ( (1.-poros(k))*csoi(k)*rhosoi(k)
     >           + poros(k)*(1.-wisoi(k))*wsoi(k)*ch2o*rhow
     >           + poros(k)*wisoi(k)*cice*rhow
     >         ) * hsoi(k)
c
        chav = ch2o*wpud + cice*wipud + rwork
c
        dh = chav * (tmelt-tsoi(k))
        dw = min (wpud, max (0., dh/hfus))
        wipud = wipud + dw
        wpud  = wpud  - dw
        chav = ch2o*wpud + cice*wipud + rwork
        tsoi(k) = tmelt - (dh-hfus*dw) / chav
c
c if any puddle ice above tmelt, melt some to puddle liquid
c
        dh = chav * (tsoi(k)-tmelt)
        dw = min (wipud, max (0., dh/hfus))
        wipud = wipud - dw
        wpud  = wpud  + dw
        chav = ch2o*wpud + cice*wipud + rwork
        tsoi(k) = tmelt + (dh-hfus*dw) / chav
c  
cc  500 continue
c
      return
      end
c
c
c ---------------------------------------------------------------------
      subroutine soilh2o (owsoi, fwtop, fwpud, fsqueez, wflo)
c ---------------------------------------------------------------------
c
c sets up call to tridia to solve implicit soil moisture eqn,
c using soil temperatures in wsoi (in comsoi)
c
c lower bc can be no h2o flow or free drainage, set by bperm below
c
      include 'implicit.h'
c
      include 'compar.h'
      include 'comsoi.h'
      include 'com1d.h'
c
c Arguments : all are supplied except wflo (returned):
c
      real owsoi(nsoilay),  ! soil moistures at start of timestep
     >     fwtop,          ! evaporation from top soil layer
     >     fwpud,          ! h2o flux into top layer (infiltration from puddle)
     >     fsqueez,        ! excess water at end of time step in soil column  
     >     wflo(nsoilay+1)  ! downward h2o flow across layer boundaries
c
c local variables
c     
      integer k,     ! loop indices
     >        km1,
     >        kka, 
     >        kkb 
c
      integer 
     >  m,                ! exponents 
     >  n
c
      real dmin,                ! minimum diffusivity for dry soils (m**2 s-1) 
     >     rimp,                ! implicit fraction of the calculation (0 to 1)
*     >     bperm,               ! = 0 for impermeable (no drainage) lower bc,
     >                          ! = 1 for free drainage lower bc
     >     zbex, z, dt, zz
c 
      real hsoim(nsoilay+1)  ! vertical distances between centers of layers
c
      real
     >  wsoim(nsoilay+1),    ! interpolated moisture values at layer boundaries
     >  wsoia(nsoilay+1),    ! 
     >  wsoib(nsoilay+1),    ! '
     >  weim(nsoilay+1),     ! '
     >  weip(nsoilay+1),     ! '
     >  a,                  ! intermediate terms (const for each pt)
     >  b,                  ! 
     >  bwn,                ! 
     >  bwn1,               ! 
     >  e(nsoilay+1),        ! intermediate terms in algebraic devel 
     >  f(nsoilay+1),        ! '
     >  g(nsoilay+1),        ! '
     >  d1(nsoilay),         ! diagonals of tridiagonal systems of equations 
     >  d2(nsoilay),         !  '
     >  d3(nsoilay),         !  '
     >  rhs(nsoilay),        ! right-hand sides of systems of equations
     >  w1(nsoilay),         ! work arrays needed by tridia
     >  w2(nsoilay)          !  '
c
      save dmin, rimp
      data dmin, rimp /1.e-9, 1.0/
c
c set lower boundary condition for the soil
c (permeability of the base)
c
c     bperm = 0.00  ! e.g. fully impermeable base
c     bperm = 1.00  ! e.g. fully permeable base
c
*      bperm = 0.10
c
c set level vertical distances, interpolated moistures, and
c interpolation weights
c
c top layer
c
      k = 1
c
cc      do 100 i = 1, npoi
c
        hsoim(k) = 0.5 * hsoi(k)
c
        weim(k) = 0.0
        weip(k) = 1.0
c
        wsoim(k) = wsoi(k)
        wsoia(k) = min (wsoim(k), 1.0)
        wsoib(k) = min (wsoim(k), 1.0)
c
cc  100 continue
c
c middle layers
c
      do 110 k = 2, nsoilay
c
cc        do 120 i = 1, npoi
c
          hsoim(k) = 0.5 * (hsoi(k-1) + hsoi(k))
c
          weim(k) = 0.5 * hsoi(k) / hsoim(k)
          weip(k) = 1.0 - weim(k)
c
          wsoim(k) = weim(k) * wsoi(k-1) + weip(k) * wsoi(k)
          wsoia(k) = min (wsoim(k), 1.0)
          wsoib(k) = min (wsoim(k), 1.0)
c
cc  120   continue
c
  110 continue
c
c bottom layer
c
      k = nsoilay + 1
c
cc      do 130 i = 1, npoi
c
        hsoim(k) = 0.5 * hsoi(k-1)
c
        weim(k) = 1.0
        weip(k) = 0.0
c
        wsoim(k) = wsoi(k-1)
        wsoia(k) = min (wsoim(k), 1.0)
        wsoib(k) = min (wsoim(k), 1.0)
c
cc  130 continue
c
c set intermediate quantities e,f,g. these are terms in the
c expressions for the fluxes at boundaries between layers,
c so are zero for k=1. use bwn1 to account for minimum 
c diffusivity dmin. bperm is used for k=nsoilay+1 to set the
c type of the lower bc.
c
c top layer
c
      k = 1
c
      e(k) = 0.0
      f(k) = 0.0
      g(k) = 0.0
c
c middle layers
c
      do 200 k = 2, nsoilay
c
cc        do 210 i = 1, npoi
c
c now that hydraul, suction and ibex can vary with depth,
c use averages of surrounding mid-layer values
c
c (see notes 8/27/93)
c
          a = weim(k) * hydraul(k-1) +
     >           weip(k) * hydraul(k  )
c
          b = weim(k) * hydraul(k-1) * 
     >           suction(k-1) * bex(k-1) +
     >           weip(k) * hydraul(k  ) *
     >           suction(k  ) * bex(k  )
c
          zbex = weim(k) * bex(k-1) +
     >           weip(k) * bex(k  ) 
c
          m = 2 * nint(zbex) + 3
          n =     nint(zbex) + 2
c
          bwn1 = b * (wsoib(k)**(n-1))
          bwn  = bwn1 * wsoib(k)
c
          if (bwn.lt.dmin) bwn1 = 0.0
          bwn = max (bwn, dmin)
c
          e(k) =  (-1.+rimp*m)*a*(wsoia(k)**m)
     >            + ((1.-rimp)*bwn - rimp*n*bwn1*wsoib(k))
     >              * (wsoi(k)-wsoi(k-1)) / hsoim(k)
c
          f(k) = - rimp*m*a*(wsoia(k)**(m-1))
     >             + rimp*n*bwn1
     >               * (wsoi(k)-wsoi(k-1)) / hsoim(k)
c
          g(k) = rimp*bwn
c
cc  210     continue
c
  200 continue
c
c bottom layer
c
      k = nsoilay + 1
c
cc      do 220 i = 1, npoi
c
        a = hydraul(nsoilay) 
        b = hydraul(nsoilay)*suction(nsoilay)*ibex(nsoilay)
c
        m = 2*ibex(nsoilay) + 3
        n = ibex(nsoilay)   + 2
c
        e(k) =                -a*(wsoia(k)**m)*bperm
        f(k) = 0.0
        g(k) = 0.0
c
cc  220 continue
c
c deduce all e,f,g in proportion to the minimum of the two 
c adjacent layers' (1-wisoi), to account for restriction of flow
c by soil ice. this will cancel in loop 300  with the factor 
c 1-wisoi in (one of) the layer's porosflo, even if wisoi=1 by 
c the use of epsilon limit. so a layer with wisoi=1 will form a 
c barrier to flow of liquid, but still have a predicted wsoi
c
      do 230 k = 1, nsoilay+1
c
        kka = max (k-1,1)
        kkb = min (k,nsoilay)
c
cc        do 240 i=1,npoi
c
c multiply by an additional factor of 1-wisoi for stability
c
          z = max(0.,1.-max(wisoi(kka),wisoi(kkb)))**2
c
          e(k) = z * e(k)
          f(k) = z * f(k)
          g(k) = z * g(k)
c
cc  240   continue
c
  230 continue
c
c set matrix diagonals and right-hand sides
c
      do 300 k = 1, nsoilay
c
cc        do 310 i = 1, npoi
c
          dt = dtime / (porosflo(k)*hsoi(k))
          d1(k) = dt*(   f(k)*0.5*hsoi(k)/hsoim(k)
     >                   - g(k)/hsoim(k) )
          rhs(k) = wsoi(k) + dt*( e(k+1) - e(k) )
c
cc  310   continue
c
        if (k.eq.1) then
c
cc          do 320 i=1,npoi
c
            dt = dtime / (porosflo(k)*hsoi(k))
            rhs(k) = rhs(k) + dt*(fwtop+fwpud)/rhow
c
cc  320     continue
c
        endif
c
        if (k.lt.nsoilay) then
c
          km1 = max (k-1,1)
c
cc          do 330 i=1,npoi
c
            dt = dtime / (porosflo(k)*hsoi(k))
            d2(k) = 1. + dt*( - f(k+1)*0.5*hsoi(k+1)/hsoim(k+1)
     >                          + f(k)  *0.5*hsoi(km1)/hsoim(k)
     >                          + g(k+1)/hsoim(k+1)
     >                          + g(k)  /hsoim(k) )
            d3(k) = dt*( - f(k+1)*0.5*hsoi(k)/hsoim(k+1)
     >                     - g(k+1)            /hsoim(k+1) )
c
cc  330     continue
c
        else if (k.eq.nsoilay) then
c
cc          do 340 i=1,npoi
c
            dt = dtime / (porosflo(k)*hsoi(k))
            d2(k) = 1. + dt*( - f(k+1)
     >                          + f(k)  *0.5*hsoi(k-1)/hsoim(k)
     >                          + g(k)  /hsoim(k) )
            d3(k) = 0.0
c
cc  340     continue
c
        endif
c
  300 continue
c
c solve the systems of equations
c
      call tridia ( nsoilay, d1,d2,d3, rhs, wsoi, w1,w2)
c
cc      do 400 i = 1, npoi
c
        fsqueez = 0.0
        wflo(nsoilay+1) = - rhow * e(nsoilay+1)
c
cc  400 continue
c
      do 500 k = nsoilay, 1, -1
c
cc        do 510 i = 1, npoi
c
          zz = rhow * poros(k) * 
     >         max(epsilon, (1.-wisoi(k))) * hsoi(k)
c
          wsoi(k) = wsoi(k) + dtime * fsqueez / zz 
          fsqueez = max (wsoi(k)-1.,0.) * zz / dtime
          wsoi(k) = min (wsoi(k),1.)
c
          wflo(k) = wflo(k+1) + (wsoi(k)-owsoi(k)) * zz / dtime
c
cc  510   continue
c
  500 continue
c
c step puddle liquid due to fsqueez and fwpud
c
c also subtract net puddle-to-top-layer flux from wflo(1),
c since puddle and top soil layer are lumped together in soilheat
c so upper wflo should be external flux only (evap/condens)

cc      do 600 i = 1, npoi
c
        wpud   = wpud   + (fsqueez - fwpud) * dtime
        wflo(1) = wflo(1) + (fsqueez - fwpud)
c
cc  600 continue
c
      return
      end
c
c
c ---------------------------------------------------------------------
      subroutine soilheat (otsoi, owsoi, c0pud, fhtop, wflo, c1pud)
c ---------------------------------------------------------------------
c
c        sets up call to tridia to solve implicit soil/ice heat 
c        conduction, using layer temperatures in tsoi (in comsoi).
c        the heat flux due to liquid flow previously calculated
c        in soilh2o is accounted for. lower bc is conductive flux = 0
c        for soil (although the flux due to liquid drainage flow can
c        be > 0)
c
      include 'implicit.h'
c
      include 'compar.h'
      include 'comsoi.h'
c
c Arguments
c
      real
     >  otsoi(nsoilay),    ! soil/ice temperatures at start of timestep (redundant
c                                 with tsoi, but passed to be consistent with soilh2o)
     >  owsoi(nsoilay),    ! soil moistures at start of timestep (before soilh2o)
     >  c0pud(nsoilay),    ! layer heat capacity due to puddles (=0 except for top)
     >  c1pud(nsoilay),    ! updated c0pud
     >  fhtop,            ! heat flux into top layer from atmos
     >  wflo(nsoilay+1)    ! downward h2o  flow across layer boundaries
c
c local variables
c
      integer k, km1, kp1    ! loop indices
c
      real rimp,                ! implicit fraction of the calculation (0 to 1)
     >     rwork, rwork1,       ! work variables
     >     rwork2, rwork3,      ! work variables
     >     t
c
      real
     >  whflo(nsoilay+1),   ! downward heat fluxes across layer bdries due to h2o
     >                           ! movement calculated in soilh2o
     >  con(nsoilay+1),     ! conduction coeffs between layers
     >  c0(nsoilay),        ! specific heats at start of timestep
     >  c1(nsoilay),        ! specific heats at end of timestep
     >  d1(nsoilay),        ! diagonals of tridiagonal systems of equations
     >  d2(nsoilay),        !  ''
     >  d3(nsoilay),        !  ''
     >  rhs(nsoilay),       ! right-hand sides of systems of equations
     >  w1(nsoilay),        ! work arrays needed by tridia
     >  w2(nsoilay)
c
      data rimp /1.0/
c
c set conduction coefficient between layers, and heat fluxes
c due to liquid transport
c
c top layer
c
      k = 1
c
cc      do 100 i = 1, npoi
        con(k) = 0.0
        whflo(k) = wflo(k) * ch2o * tsoi(k)
cc  100 continue
c
c middle layers
c
      do 110 k = 2, nsoilay
c
cc        do 120 i = 1, npoi
c
          con(k) =  1. / (0.5 * (hsoi(k-1) / consoi(k-1) +
     >                             hsoi(k)   / consoi(k)))
c
          t = (hsoi(k) * tsoi(k-1) + hsoi(k-1) * tsoi(k)) /
     >        (hsoi(k-1)             + hsoi(k))
c
          whflo(k) = wflo(k) * ch2o * t
c
cc  120   continue
c
  110 continue
c
c bottom layer
c
      k = nsoilay + 1
c
cc      do 130 i = 1, npoi
        con(k) = 0.0
        whflo(k) = wflo(k) * ch2o * tsoi(k-1)
cc  130 continue
c
c set diagonals of matrix and right-hand side. use old and
c new heat capacities c0, c1 consistently with moisture fluxes
c whflo computed above, to conserve heat associated with 
c changing h2o amounts in each layer
c
      do 200 k = 1, nsoilay
c
        km1 = max (k-1,1)
        kp1 = min (k+1,nsoilay)
c
cc        do 210 i=1,npoi
c
          rwork1 = (1.-poros(k))*csoi(k)*rhosoi(k)
          rwork2 = poros(k)*(1.-wisoi(k))*ch2o*rhow
          rwork3 = poros(k)*wisoi(k)*cice*rhow
c
          c0(k) = c0pud(k) + 
     >              (   rwork1
     >                + rwork2 * owsoi(k)
     >                + rwork3
     >              ) * hsoi(k)
c
          c1(k) = c1pud(k) +  
     >              (   rwork1
     >                + rwork2 * wsoi(k)
     >                + rwork3
     >              ) * hsoi(k)
c
          rwork = dtime/c1(k)
c
          d1(k) =    - rwork * rimp * con(k)
          d2(k) = 1. + rwork * rimp * (con(k)+con(k+1))
          d3(k) =    - rwork * rimp * con(k+1)
c
          rhs(k) = (c0(k)/c1(k))*tsoi(k) + rwork
     >               * ( (1.-rimp)*con(k)  *(tsoi(km1)-tsoi(k))
     >                 + (1.-rimp)*con(k+1)*(tsoi(kp1)-tsoi(k))
     >                 + whflo(k) - whflo(k+1) )
c
cc  210   continue
c
        if (k.eq.1) then
cc          do 220 i=1,npoi
            rhs(k) = rhs(k) + (dtime/c1(k))*fhtop
cc  220     continue
        endif
c
  200 continue
c
c solve systems of equations
c
      call tridia (nsoilay, d1,d2,d3, rhs, tsoi, w1,w2)
c
c deduce downward heat fluxes between layers
c
cc      call scopy (1, fhtop, hflo(1))
 	 hflo(1) = fhtop
c
      do 300 k=1,nsoilay
cc        do 310 i=1,npoi
          hflo(k+1) = hflo(k) -
     >                  (c1(k)*tsoi(k) - c0(k)*otsoi(k)) / dtime
cc  310   continue
  300 continue
c
      return
      end
c
c
c ---------------------------------------------------------------------
      subroutine wadjust
c ---------------------------------------------------------------------
c
c set wsoi, wisoi to exactly 0 if differ by negligible amount, 
c to protect epsilon logic in soilctl and soilh2o
c
c ice-liquid transformations in soilctl loop 400 can produce very
c small -ve amounts due to roundoff error, and very small -ve or +ve
c amounts can cause (harmless) "underflow" fpes in soilh2o
c
      include 'implicit.h'
c
      include 'compar.h'
      include 'comhyd.h'
      include 'comsoi.h'
      include 'com1d.h'
c 
c local variables
c
      integer k
      real ztot0, ztot1
c
      do 100 k = 1, nsoilay
cc        do 110 i = 1, npoi
c
c initial total soil water
c
         ztot0 = hsoi(k) * poros(k) * rhow *
     >           ((1. - wisoi(k)) * wsoi(k) + wisoi(k))
c
c set bounds on wsoi and wisoi
c
         if (wsoi(k).lt.epsilon)  wsoi(k)  = 0.0
         if (wisoi(k).lt.epsilon) wisoi(k) = 0.0
c
         wsoi(k)  = min (1., wsoi(k))
         wisoi(k) = min (1., wisoi(k))
c
         if (wisoi(k).ge.1-epsilon) wsoi(k) = 0.0
c
c for diagnosis of total adjustment
c
         ztot1 = hsoi(k) * poros(k) * rhow *
     >           ((1. - wisoi(k)) * wsoi(k) + wisoi(k))
c
         gadjust = gadjust + (ztot1 - ztot0) / dtime
c
cc  110   continue
  100 continue
c
      return
      end	 